...
public class P15_2 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    int[][] grid = new int[5*data1.size()][5*data1.get(0).length()];
    for (int i = 0; i < grid.length; i++) {
      int m = i / data1.size();
      for (int j = 0; j < grid[0].length; j++) {
        int n = j / data1.get(0).length();
        int r = i%data1.size();
        int c = j%data1.get(0).length();
        //System.out.println(r + " " + c);
        grid[i][j] = Integer.parseInt(data1.get(r).substring(c,c+1)) + n + m;
        if (grid[i][j] > 9) {
          grid[i][j] -= 9;
        }
      }
